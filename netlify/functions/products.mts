...
const CATEGORIES = ["Femme", "Homme", "Streetwear", "Luxe", "Archive", "Sneakers", "Accessoires"];
const CONDITIONS = ["Neuf avec étiquette", "Excellent état", "Très bon état", "Bon état"];
...
export default async (req: Request) => {
  ...
  if (req.method === "GET") {
    ...
    for (const id of idx.slice(-60).reverse()) {
      const p = (await products.get(`p:${id}`, { type: "json" })) as Record<string, unknown> | null;
      if (!p || p.shadow) continue; // shadows = seeds vendus, exposés via /api/sold
      out.push({ ...p, mine: me ? p.sellerId === me.id : false });
    }
    // état vendu des pièces seed (shadows) pour griser le catalogue côté UI
    const soldSeeds = ((await products.get("sold-seeds", { type: "json" })) as string[]) ?? [];
  ...
  if (!name || !brand || !size) return bad("Titre, marque et taille sont requis");
